...
while True:
    url = input("輸入url的連結:（預設值 https://www.flickr.com/photos/akevchiu/galleries/72157722210730785/): ")
    if url != '':
        break

folder = input("請輸入下載目錄名稱 (./Expo/<名稱>): ")
path = '../Expo/' + folder
# num = int(input("輸入下載張數 (以 10張 為準): "))
# total = num/10
...
for ph in photo:
	r = requests.get(ph, headers=headers)   	# 開始抓圖片
